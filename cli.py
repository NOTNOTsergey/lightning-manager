...
def backup(dates):
    ...
    if dates:
        if len(os.listdir("./backups")) != 1:
            print(colorama.Fore.YELLOW, f'there is {len(os.listdir("./backups"))} backups', '\ndates:\n', sep='')
        else:
            print(colorama.Fore.YELLOW, f'there is only 1 backup', '\ndate:\n', sep='')
        ...
        for i in os.listdir('./backups'):
            ...
            print(colorama.Fore.LIGHTCYAN_EX, i, colorama.Fore.BLUE, ' : ', colorama.Fore.LIGHTMAGENTA_EX, read_backup(i)[2], sep='')
    elif len(os.listdir('./backups')) >= 5:
        print(colorama.Fore.RED, 'backups number is larger then 5, i will remove and replace old backups')

        backups = [i for i in map(str, sorted([i for i in map(int, os.listdir('./backups'))])) ]

        for i in range(4, len(backups)):
            os.remove( os.path.join(os.path.abspath(''), 'backups/' + backups[i]) )
...
def restore(latest, oldest, number):
        ...
        if symmetric_encrypt_str('hi', passwd) == db[1]['auth'][0] and hashlib.sha3_256(username.encode()).hexdigest() in db[0]:
            ...
            if latest and oldest:
                print(colorama.Fore.RED, 'i cant restore latest and oldest backups at once, choose what you need ...')
                return
